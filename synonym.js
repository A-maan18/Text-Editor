 ...
 tinymce.PluginManager.add('synonym', function(editor, url) {
  ...
  editor.fetchSynonyms =  function(searchTerm){  
    ...
    resultsDOM.innerHTML = "<div class='loader-wrapper'><div class='spinner'></div></div>";
    xhttp.onreadystatechange = function() {
      if (this.readyState == 4 && this.status == 200) {
         try{
            editor.populateSynonymsResults(JSON.parse(this.responseText),resultsDOM);
         }catch{
  ...
  editor.populateSynonymsResults = function(resultsArr,resultsDOM){ 
    if(resultsDOM) {
      ...
      }else resultsDOM.innerHTML= "Something Went Wrong.. Please Try Again...";
    }
  };
